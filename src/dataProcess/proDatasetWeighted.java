...
public class proDatasetWeighted 
{
	...
	public static HashMap<Integer, Integer> array2 = new HashMap<Integer, Integer>();  //<点, 所在超点 >
	...
	public void readTxtFile2(String filePath)
	{
		try 
        {
            String encoding="GBK";
            ...
            { //判断文件是否存在
                ...
                BufferedReader bufferedReader = new BufferedReader(read);
                    ...
                    while((lineTxt = bufferedReader.readLine()) != null)
                    {
                    	//User.add(lineTxt);
                    	String[] mm = lineTxt.split("\t");
                    	int m0 = Integer.parseInt(mm[0].trim());
                    	int m1 = Integer.parseInt(mm[1].trim());
                    	array2.put(m0, m1);
                    }
                    read.close(); 
        }else{
	...
	public void makeGraph()   //将将fromId,toId转化成边
	{
		...
		for(int vid:vertexId2)
		{
			if(array.containsKey(vid))
			{
				...
				for(Map.Entry<Integer,Float> toidW:toids.entrySet())
				{
					...
					Edge e = addEdge(realvid, realvid2);
					e.weight = weight;
		...
		{
			int realvid = NameId.get(entry.getKey());
			vertexList.get(realvid).communityId.add(entry.getValue());
		}*/
